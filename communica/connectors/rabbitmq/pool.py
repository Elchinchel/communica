...
class ChannelRoute:
    ...
    def __init__(self, chan: PooledChannel, exchange_name: str, routing_key: str) -> None:
        self.chan = chan
        ...
        with self._thread_lock:
            if loop not in self._loop_contexts:
                self._loop_contexts[loop] = self._LoopContext(
                    defaultdict(lambda: asyncio.Lock()), dict(), set()
                )
